    ...
    return (
        <button
            ...
            style={{
                ...
                textAlign: "left", fontFamily: T.font, transition: "all 0.15s ease",
            }}
            onMouseEnter={e => { if (!active) e.currentTarget.style.background = T.slate100; }}
            onMouseLeave={e => { if (!active) e.currentTarget.style.background = "transparent"; }}
        >
            <FontAwesomeIcon icon={icon} style={{ fontSize: 16, color: active ? T.green : T.slate500 }} />
            <span style={{ flex: 1 }}>{label}</span>
            {badge > 0 && (
                <span style={{
                    background: T.green, color: T.white, borderRadius: 99,
    ...
    return (
        <>
            ...
            <aside style={{
                ...
                background: T.white, borderRight: `1px solid ${T.slate100}`,
                boxShadow: "4px 0 24px rgba(0,0,0,0.08)",
                height: "calc(100vh - 65px)", position: "fixed",
                top: 65, left: 0, zIndex: 50, overflowY: "auto",
            }}>
                <div style={{
                    padding: "20px 16px 16px", borderBottom: `1px solid ${T.slate100}`,
                    display: "flex", alignItems: "center", gap: 12,
                }}>
                    <div style={{
                        width: 44, height: 44, borderRadius: 12, background: T.slate900,
                        flexShrink: 0, overflow: "hidden",
                        display: "flex", alignItems: "center", justifyContent: "center",
                    }}>
                        <img src={avatarSrc} alt={customer?.name || "Customer"}
