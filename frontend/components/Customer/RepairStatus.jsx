    ...
    return (
        <div className="space-y-5">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-5">
                        ...
                        <img
                            ...
                            alt="Toyota Prius"
                            className="h-20 w-28 rounded-xl object-cover ring-2 ring-[#16a34a]/20"
                        />
                        <div>
                            ...
                            <p className="mt-1 text-sm font-mono text-[#274c3a]/70">Service: <span className="font-semibold text-[#14532d]">Engine Overheating</span></p>
                            {/*  API: Replace with repair.requestId */}
                            <p className="mt-1 text-sm font-mono text-[#274c3a]/70">Request ID: <span className="font-semibold text-[#14532d]">FXG-001</span></p>
                        </div>
                    </div>
                    <div className="text-right text-sm font-mono text-[#274c3a]/60">
                        <p>Requested on</p>
                        {/*  API: Replace with formatted repair.requestedAt */}
                        <p className="mt-1 font-semibold text-[#14532d]">May 25, 2026 • 09:15 AM</p>
                    </div>
                </div>
            </section>
            ...
            <section className="rounded-[28px] border border-[#d1e7d7] bg-white p-6 shadow-[0_4px_12px_rgb(22,163,74,0.06)]">
                            ...
                            <div key={step.key} className="flex flex-1 flex-col items-center">
                                <div className="relative flex w-full items-center justify-center">
                                    ...
                                    <div className="relative z-10">
                                        {step.status === "done"    && <div className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-[#16a34a] bg-[#16a34a]/10"><FontAwesomeIcon icon={step.icon} className="text-xl text-[#16a34a]" /></div>}
                                        {step.status === "active"  && <div className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-[#0d9488] bg-[#0d9488]/10"><FontAwesomeIcon icon={step.icon} className="text-xl text-[#0d9488]" /></div>}
                                        {step.status === "pending" && <div className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-[#d1e7d7] bg-white"><FontAwesomeIcon icon={step.icon} className="text-xl text-[#d1e7d7]" /></div>}
                                        {step.status === "done" && (
                                ...
                                <div className="mt-4 w-full px-1 text-center">
                                    <p className="text-sm font-mono font-semibold text-[#14532d]">{step.label}</p>
                                    {/*  API: step.date and step.time come from the backend (null if not reached yet) */}
                                    {step.date
                                        ? <><p className="mt-1 text-xs font-mono text-[#274c3a]/60">{step.date}</p><p className="text-xs font-mono text-[#274c3a]/60">{step.time}</p></>
                                        : <p className="mt-1 text-xs tracking-widest text-[#d1e7d7]">- - - - - -</p>
                                    }
                                    <p className="mt-2 text-xs font-mono leading-4 text-[#274c3a]/60">{step.desc}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                {/* Reassurance banner */}
                <div className="mt-8 flex items-center gap-4 rounded-2xl border border-[#d1e7d7] bg-[#f0f7f2] px-5 py-4">
            ...
            <section className="rounded-[28px] border border-[#d1e7d7] bg-white p-6 shadow-[0_4px_12px_rgb(22,163,74,0.06)]">
                <h3 className="text-base font-semibold text-[#14532d]">Repair Details</h3>
