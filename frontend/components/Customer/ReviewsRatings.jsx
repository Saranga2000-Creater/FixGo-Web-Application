// ============================================================
// FILE: ReviewsRatings.jsx
// PURPOSE: Reviews & Ratings page — rating summary + review list.
//
// 👉 API: GET /api/customer/reviews/summary
//         Response: { averageRating, totalReviews,
//                     breakdown: [{ stars, count, pct }] }
// 👉 API: GET /api/customer/reviews?filter=alltime
...
const ACCENT_STYLES = {
	...
	yellow: { bg: "bg-[#fffbeb]", text: "text-[#d97706]" },
};
	...
	return (
		<div className="flex items-center gap-0.5">
			{[1, 2, 3, 4, 5].map((s) => (
				<span
					key={s}
					className={`${sizeClass} ${rating >= s || (rating >= s - 0.5 && rating < s) ? "text-amber-400" : "text-slate-200"}`}
				>★</span>
			))}
		</div>
	);
