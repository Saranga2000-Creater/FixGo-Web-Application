...
function RegRow({ shop, isLast, onApprove, onReview }) {
  const flagColor = {
    Verified: "text-green-600",
    "IP Match": "text-[#FF6B1A]",
    "Pending Scan": "text-gray-400",
  };
  const initials = (shop.shopName || "?").split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 2);

  return (
    <div className={`grid gap-4 items-center py-3.5 px-6 [grid-template-columns:2fr_1.5fr_1fr_1fr] ${!isLast ? "border-b border-gray-100" : ""}`}>
      <div className="flex items-center gap-3">
        <div className="w-[38px] h-[38px] rounded-[10px] bg-gray-100 flex items-center justify-center text-[13px] font-bold text-gray-500 shrink-0">{initials}</div>
        <div>
          <p className="text-[13px] font-bold text-gray-900 m-0">{shop.shopName}</p>
          <p className="text-[11px] text-gray-400 m-0">{shop.email}</p>
        </div>
      </div>
      <div className="flex flex-wrap gap-1">
        <span className="rounded-md border border-gray-200 bg-gray-50 py-0.5 px-2 text-[11px] font-semibold text-gray-500">{shop.shopCategory || "Unknown"}</span>
      </div>
      <div>
        <span className={`flex items-center gap-1.5 text-xs font-semibold ${flagColor["Verified"]}`}>
      ...
      <div className="flex gap-2">
        <button
          ...
          className="rounded-[10px] bg-green-600 text-white border-none py-2 px-3 text-xs font-bold cursor-pointer hover:bg-green-700 transition-colors"
        >
          Approve
        </button>
        <button
          onClick={() => onReview(shop)}
          className="rounded-[10px] bg-white text-gray-700 border border-gray-200 py-2 px-3 text-xs font-bold cursor-pointer hover:bg-gray-50 transition-colors"
        >
          Review
        </button>
      </div>
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {activeTab === "registration" && (
        <PageCard>
          <TableHeader
            cols={["Shop / Email", "Category", "Documents", "Actions"]}
            gridCols="[grid-template-columns:2fr_1.5fr_1fr_1fr]"
          />
          {regLoading ? (
            <EmptyState icon={<FontAwesomeIcon icon={faSpinner} spin className="text-3xl" />} message="Loading…" />
          ) : filteredReg.length === 0 ? (
            <EmptyState icon={<FontAwesomeIcon icon={faCircleCheck} className="text-3xl text-green-400" />} message="No pending registrations." />
          ) : (
